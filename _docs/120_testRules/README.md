...
- 各 Domain には少なくとも 1 つ以上の domain 近傍 UT を持つ
- 値オブジェクトまたはエンティティの不変条件を直接検証する
- Application test だけで Domain の正しさを代替しない
...
特に Domain 層は、実装ファイルだけ存在して `*.spec.*` が1つもない状態を禁止する。
...
`tests/it/...` では placeholder test を禁止する。

差し戻し対象の例:

- `assert.equal(true, true)`
- `expect(true).toBe(true)`
- smoke test の名目で契約を何も検証していないもの
...
- テスト名だけでも意図が分かる
- 内側の層は外側の都合に依存しない
- Story が大きな塊ではなく、小さな UI 契約から積み上がっている
...
- Domain の振る舞いを Application test だけで代用している
- Storybook が大きな form や page 断片だけで構成され、小さな component 契約が見えない
- placeholder だけで存在している IT
